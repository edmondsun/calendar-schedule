    ...
    return {
      options: {
        ...
        eventDelete: function(calEvent, element, dayFreeBusyManager, 
                                                      calendar, clickEvent) {
            calendar.weekCalendar('removeEvent',calEvent.start);
	},
        ...
        eventHeader: function(calEvent, calendar) {
          ...
          if (displayTitleWithTime) {
            ...
            // Avoid the 24:00:00 transform to 00:00:00.
            if (calEvent.end.getHours() === 0 && calEvent.end.getMinutes() === 0 && calEvent.end.getSeconds() === 0) {
              calEvent.end.setTime(calEvent.end.getTime() - 1);
            }
          ...
          if( (calEvent.end - calEvent.start) > 3600000 ){
      ...
      /*
        ...
        */
      removeEvent: function(eventStart) {

          var self = this;

          self.element.find('.wc-cal-event').each(function() {
            if ($(this).data('calEvent').start === eventStart) {
                $(this).remove();
                return false;
            }
          });
      ...
      /*
        * select all event schedule in the calendar.
        */
      schedule: function(calEvent) {

          var date = this.options.daysToShow;

          for (var x=0; x < date; x+=1) {

            var dayEvent = this.element.find('.wc-full-height-column')[x];
            var $target = $(dayEvent);
            if ($target.hasClass('wc-day-column-inner')) {
              var $newEvent = $('<div class=\"wc-cal-event wc-new-cal-event wc-new-cal-event-creating\"></div>');
              $newEvent.css({lineHeight: (this.options.timeslotHeight - 2) + 'px', fontSize: (this.options.timeslotHeight / 2) + 'px'});
              $target.append($newEvent);
              $newEvent.css({height: this.options.timeslotHeight * this.options.maxEventLength}).show();
              var top = 0;
              var eventDuration = this._getEventDurationFromPositionedEventElement($target, $newEvent, top);
              var newCalEvent = {start: eventDuration.start, end: eventDuration.end, title: this.options.newEventText};
              $newEvent.remove();
              this._renderEvent(newCalEvent, $target);
            }
          }
      },

      /*
        * save the event in the calandar.
        * this is use for recroding all event for menu list.
        */
      saveEvent: function(calEvent) {

          var self = this,
              eventCnt = self.element.find('.wc-cal-event').length,
              eventStart,
              eventEnd,
              saveStarTime,
              saveEndTime;
          ...
          for (var x=0; x < date; x+=1) {
            ...
            var dayEvent = self.element.find('.wc-full-height-column')[x];
            var $dayTestEvent = $(self.element.find('.wc-full-height-column')[x]);
            var scheduleDate = {
                                 0 : gLang.L_WeekDay_Sunday,
                                 1 : gLang.L_WeekDay_Monday,
                                 2 : gLang.L_WeekDay_Tuesday,
                                 3 : gLang.L_WeekDay_Wednesday,
                                 4 : gLang.L_WeekDay_Thursday,
                                 5 : gLang.L_WeekDay_Friday,
                                 6 : gLang.L_WeekDay_Saturday
                               };

            if (dayEvent.children.length > 10) {
              alert(scheduleDate[x] + " :" +gLang.L_Schedule_Over);
              opener.closeSchedule[x] = false;
            }

            if(opener.closeSchedule[x] == true){

              for (var y=0; y < dayEvent.children.length; y+=1) {

              eventStart = $($dayTestEvent.children()[y]).find('.wc-time').html().split('<')[0].split(' - ')[0];
              eventEnd   = $($dayTestEvent.children()[y]).find('.wc-time').html().split('<')[0].split(' - ')[1];

              saveStarTime = this._setTimer(eventStart.split(':')[0])   + ":" + 
                             this._setTimer(eventStart.split(':')[1]);

              saveEndTime  = this._setTimer(eventEnd.split(':')[0])   + ":" + 
                             this._setTimer(eventEnd.split(':')[1]);  

              opener.eventDatasDay.push(x);  
              opener.eventDatasStar.push(saveStarTime);
              opener.eventDatasEnd.push(saveEndTime);
      ...
      _renderCalendarButtons: function($calendarContainer) {
        ...
        if (options.buttons) {
            ...
            calendarNavHtml += '<div class=\"ui-widget-header wc-toolbar\">';
              calendarNavHtml += '<div class=\"wc-display\">';
              calendarNavHtml += '<button class=\"wc-disk\">' + options.buttonText.disk + '</button>';
              calendarNavHtml += '</div>';

              calendarNavHtml += '<div class=\"wc-nav\">';
                calendarNavHtml += '<button class=\"wc-schedule\">' + options.buttonText.schedule + '</button>';
                calendarNavHtml += '<button class=\"wc-today\">' + options.buttonText.home + '</button>';
                calendarNavHtml += '<button class=\"wc-trash\">' + options.buttonText.trash + '</button>';
              calendarNavHtml += '</div>';
            ...
            $calendarContainer.find('.wc-nav .wc-schedule')
              .button({
                text: false,
            ...
            $calendarContainer.find('.wc-display .wc-disk')
              .button({
                text: false,
                icons: {primary: 'ui-icon-disk'}})
              .click(function() {
                  var cl = false;
                  opener.closeSchedule = [];

                  for (var i=0; i < 7; i++) {
                    opener.closeSchedule[i] = true;
                  }

                  self.saveEvent();
                  opener.scheduleCgi = opener.ParserSchedule();

                  for (var i=0; i < 7; i++) {
                    if (opener.closeSchedule[i] == false){
                        cl = true;  
                        break;
                    }
                  }
            ...
            $calendarContainer.find('.wc-nav .wc-trash')
              .button({
                text: false,
      ...
      _renderCalendarBodyEvents: function($calendarTableTbody) {
        ...
        for (var i = start; i < end; i++) {
          ...
          renderRow += '<div class=\"wc-time-header-cell\">' + self._hourForIndex(i) + '<span class=\"wc-am-pm\" font size="1">' + self._amOrPm(i) + '</span></div>';
          renderRow += '</div>';
        }
        renderRow += '</td>';
      ...
      _updateDayColumnHeader: function($weekDayColumns) {
          ...
          if (this.options.title) {
            ...
            $('.wc-toolbar .wc-title', self.element).html("Schedule");
          }
      },
      ...
      _renderEvents: function(data, $weekDayColumns) {
        ...
        $.each(eventsToRender, function(i, calEvent) {
          ...
          var calStarDate = calEvent.start.getDate();
          var calEndDate  = calEvent.end.getDate();

          while (startDate < endDate) {
            calEvent.start = start;
 
            if (calStarDate <= calEndDate) {
      ...
      _adjustOverlappingEvents: function($weekDay) {
          ...
          if (self.options.allowCalEventOverlap) {
            ...
            var eventWidth = $events.width() / $events.parent().width() * 100;
            // delete the event list as overlapping
            if (eventWidth != 100) {
              while (($events.length > 1) && ($events.data('calEvent') != undefined)) {
                var freeBusyManager = this.getFreeBusyManagerForEvent($events.data('calEvent'));
                this.options.eventDelete($events.data('calEvent'), $events, freeBusyManager, this.element, event);  
              }
            }
      ...
      _getEventDurationFromPositionedEventElement: function($weekDay, $calEvent, top) {
          ...
          if (end.getHours() === 0 && end.getMinutes() === 0 && end.getSeconds() === 0) {
            var end = new Date(start.getTime() + ($calEvent.height() / options.timeslotHeight) * options.millisPerTimeslot - 1);
          } else {
            var end = new Date(start.getTime() + ($calEvent.height() / options.timeslotHeight) * options.millisPerTimeslot);
          }            
      ...
      /*
        * Let schedule time is filled zero when its single number. 
        */
      _setTimer: function(timerVal) {
        var timerString;

        if (timerVal.toString().length == 1) {
          timerString = '0' + timerVal;
        } else {
          timerString = timerVal;
        }
	  ...
	  if (!this.options.readonly &&
		 ...
		 this.options.deletable(calEvent,$calEvent)) {
          ...
          if ( (calEvent.end - calEvent.start) >= 7200000 ) {
            $('.ui-resizable-handle', $calEvent).text('=');
            $('.wc-title', $calEvent).text("New Event");
          } else { 
            $('.ui-resizable-handle', $calEvent).text('');
            $('.wc-title', $calEvent).text('');
          }
